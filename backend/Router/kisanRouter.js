  ...
  if(req.body.transaction && req.body.transaction.backDate && req.body.transaction.backDate.length>0) {
    ...
    if(kisanDetails?.transactions?.length>0){
      ...
      if(kisantransactionsForTheBackDate?.length>0){
        ...
        const max_date = kisantransactionsForTheBackDate.reduce((a, b) => moment(a.date).isAfter(b.date) ? a : b).date
  ...
  if(req.body.transaction && req.body.transaction.purchaserId && req.body.transaction.purchaserId !== "") {
    ...
    if(req.body.transaction && req.body.transaction.backDate && req.body.transaction.backDate.length>0) {
      ...
      */
      ...
      const purchaserTxns = await purchaserController.controller("FindByID", req.body.transaction.purchaserId)
      if(purchaserTxns?.transactions?.length > 0) {
        ...
        if(purchaserTransactionsOfParticularBackDate?.length>0) {
          const max_date = purchaserTransactionsOfParticularBackDate.reduce((a, b) => moment(a.date).isAfter(b.date) ? a : b).date
          //console.log("max_date", max_date)
          const max_date_opr = new Date(max_date);
          max_date_opr.setSeconds(max_date_opr.getSeconds() + 1);
    ...
    purchaserDataGenerated =  await purchaserController.controller("AddTransaction", {
      ...
      transaction: { ...req.body.transaction, date: purchaser_date, _id: id, creationDate: new Date()},
    });
  }
