...
import { Card, Col, Row, Select, DatePicker, Spin, Typography, Alert } from 'antd';
...
const PurchaserReportLegacy = () => {
  ...
  useEffect(() => {
    document.title = 'VVMS - Purchaser Report (Legacy)';
    ...
    <div style={{ padding: '24px' }}>
      ...
      >
        <Text style={{ color: '#856404' }}>
          🚨 <strong>Legacy Mode:</strong> This is the original purchaser report view for comparison
          purposes.
        </Text>
      </div>

      {/* Simple Filter Section - Legacy Style */}
      <Card title="Purchaser Report Filters" style={{ marginBottom: 16 }}>
        <Row gutter={16}>
          <Col span={6}>
      ...
      {!state.isLoading && hasTransactions && (
        <Card title={`Purchaser Report for ${displayDate}`} style={{ marginBottom: 16 }}>
          <Text>
      ...
      {!state.isLoading && !hasTransactions && state.transactions !== null && (
        <Card style={{ textAlign: 'center', padding: '48px 24px' }}>
          ...
          <Text type="secondary">
            No purchaser transactions were found for the selected criteria.
          </Text>
        </Card>
      )}
    </div>
...
export default PurchaserReportLegacy;
